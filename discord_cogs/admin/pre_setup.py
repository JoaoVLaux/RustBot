...
# Creates a new text channel
    ...
    """
        ...
        if bot_cmd_channel != None:
            ...
            embed_text = f"🔒 Certifique-se de ajustar as configurações de função para\n<#{category_admin_id}>\npara restringir a visibilidade do canal a usuários autorizados,\em vez de permitir que ele fique visível para todos. 🔓"
            embed = discord.Embed(title="Atenção!",description=embed_text, color=0x8080ff)
            ...
            description=f"""
Configurações do servidor > Intergação > Bot > Definir comandos

            > Que apenas os usuários com a função <@&{bot_admin_role.id}>
            > pode ver o **commands:**
            
            {text}"""


            embed = discord.Embed(title="/Commands setar manual",
    ...
    """
        ...
        if was_created_list_len != 0:
            ...
            embed = discord.Embed(title=f"Os seguintes canais principais do sistema foram criados:",
