...
# Creates a new Role
        ...
        if rust_ultras_role != None:
            print(f"A role {rust_ultras_role.name} Já existe.")
        else:
            print(f"A {role_name} não existe.")
            rust_ultras_role = await guild.create_role(name=role_name, colour=role_colour)
            print(f"A role {rust_ultras_role.name} foi criada.")
...
# Creates a new text channel
        ...
        if server_stats_channel != None:
            ...
            embed.add_field(name="```/change_server```",
                            value="Altere o servidor ao qual o bot deve estar conectado inserindo o ID Battlemetrics do servidor",
                            inline=False)
...
# Creates a new text channel
        ...
        if rust_info != None:
            print(f"O channel {rust_info.name} já existe.")
        else:
            print(f"Ochannel {rust_info_name} Não existe.")
            ...
            print(f"O channel {rust_info.name} foi criado.")
...
# Creates a new text channel
        ...
        if rust_team_text_channel != None:
            ...
            embed = discord.Embed(colour=0xffffff, description=f"**Chat Exclusivo para <@&{rust_ultras_role_id}>. 🔒**\n> Calls do aquipe. 💥\n> **Dominar o server!🚀**\n#PatoelhoLiveLorger")
            embed.set_thumbnail(url=img_url.piktogramm.text_talk)
            embed.set_author(name="🛡️ Bem-vindo ao canal de comunicação do clan! 🛠️")
        ...
        if was_created_list_len != 0:
            ...
            embed = discord.Embed(title=f"Os seguintes canais foram criados:",
