...
class New_player(commands.Cog):
    ...
    @app_commands.command(name="add_player", description="Adiciona player na lista de observação")
    @app_commands.describe(
        player_name="Rust Player_name ou Steam_id",
        player_note="Anotação",
    )
    async def choise_team(self, interaction: discord.Interaction, player_name: str, player_note: str):
        ...
        embed = discord.Embed(title="Adicionando player...", description=f"{thinking} Thinking...", color=discord.Color.blurple())
        ...
        if player_id == None:
            ...
            embed = discord.Embed(title=f"Sem resultado para {player_name}", url=s_url, description="Tente encontrar o jogador adicionando o ID à próxima solicitação com `id:{battlemetrics_player_id}`."+f"\n{delt_str}", color=0xff0000)
            ...
            try:
                ...
                if Player_server_data == "Limite de taxa excedido":
                    while True:
                        Player_server_data = Player_Server_info(player_id, battlemetrics_server_id)
                        if Player_server_data != "Limite de taxa excedido":
                            break
            ...
            if player_online_status == False or player_online_status == "never_played":
                if player_online_status == "never_played":
                    ...
                    value = f"❌ `Nunca jogou nesse servidor` ❌ | note: `{player_note}`"
                    embed.add_field(name=Player_name, value=value, inline=True)
                else:
                    embed = discord.Embed(title=f"{Player_name}", url=f"https://www.battlemetrics.com/players/{player_id}",description=("🔴 | ID: "+str(player_id)), color=0xff0000)
                    embed.add_field(name="Ultima vez",value=f"{player_lastSeen}", inline=True)
                    embed.add_field(name="Notação", value=f"{player_note}", inline=True)
                    embed.set_footer(text=f"Selecione time:")
            else:
                ...
                embed.add_field(name="Online desde", value=f"{player_lastSeen}", inline=True)
                embed.add_field(name="Notação", value=f"{player_note}", inline=True)
                embed.set_footer(text=f"Selecione time:")
            ...
            x = -1
            options = [discord.SelectOption(label=f"🆕Criar novo time.🆕", description=f"Adicione uma novo time."),]
            while True:
                x = x + 1
                if x == Team_list_len:
                    break
            ...
            async def my_callback(interaction):
                ...
                if f"{select.values[0]}" == f"🆕Criar novo time.🆕":
                    ...
                    embed = discord.Embed(
                        title=f"{Player_name}", description="Adicionado para time", color=0xff8040)
...
class modal_New_team(ui.Modal, title="Novo Time",):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        print("Enviar Discordembed: Test Result")
        await view.wait()
        if view.value is None:
            ...
            print(f'Confirmado... self.confirm_Button = {self.confirm_Button}')
            ...
            self.confirm_Button = False
            print(f'Cancelado... self.confirm_Button = {self.confirm_Button}')
...
class clear_observationlist(commands.Cog):
    ...
    @app_commands.command(name="clear_observationlist", description="Exclui a lista Playerwatch.")
...
class modal_confrim_delt(ui.Modal, title="Limpar observationlist"):

    confirm_delt = ui.TextInput(label="Confirme com Sim", style=discord.TextStyle.short, placeholder="Confirme com Sim", required=True,)

    print("Send modal_confrim_delt: Confirme com Sim ")

    async def on_submit(self, interaction: discord.Interaction):
        confirm_delt = (str(self.confirm_delt)).lower()
        if str(confirm_delt) == "Sim":
            ...
            while True:
                ...
                try:
                    ...
                    await msg.delete()
                    print(f"Sem msg delt", "blue")
                except:
                    print(f"Sem msg to delt", "blue")

            dictionary = {}
            fill_json_file(player_observation_data_dir, dictionary)
            embed = discord.Embed(
                title="observationlist", description="observationlist foi limpa.", color=0xc0c0c0)
            await interaction.response.send_message(embed=embed, ephemeral=True)
...
class Delt_player(commands.Cog):
    ...
    @app_commands.command(name="delt_player", description="Deleta players de observationlist.")
    async def choise_player_to_delt(self, interaction: discord.Interaction):
        ...
        async def my_callback(interaction: discord.Interaction):
            ...
            embed = discord.Embed(title=f"🚮 Seria {name} para ser deletado? 🚮")
        ...
        view = View()
        view.add_item(select)
        embed = discord.Embed(title="Deletar Player", description="Selecione Player!", color=0xff8040)
        await interaction.response.send_message(embed=embed, ephemeral=True, view=view)
...
class Delt_Team(commands.Cog):
    ...
    @app_commands.command(name="delt_team", description="Exclua a time da observationlist.")
    async def choise_Team_to_delt(self, interaction: discord.Interaction):
        ...
        async def my_callback(interaction: discord.Interaction):
            ...
            embed = discord.Embed(title=f"🚮 Seria {Team_name} para ser deletado? 🚮")
            embed.add_field(name="Note", value=note, inline=True)
        ...
        view = View()
        view.add_item(select)
        embed = discord.Embed(title="Delete player", description="Selecione um player!", color=0xff8040)
        await interaction.response.send_message(embed=embed, ephemeral=True, view=view)
                        ...
                        while True:
                            ...
                            if online == True:
                                embed_New_Status = discord.Embed(
                                    title="🟢 Novo Online Status 🟢", description=f"Team `{Team_Name}` is now online! <#{player_observation_channel_id}>", color=0xff8000)
                                embed_list.append(embed_New_Status)
                                embed_list.append(embed)
                                await User.send(embeds=embed_list)
                            else:
                                embed_New_Status = discord.Embed(
                                    title="🔴 Novo Online Status 🔴", description=f"Team `{Team_Name}` is now offline! <#{player_observation_channel_id}>", color=0xff0000)
                                embed_list.append(embed_New_Status)
                                embed_list.append(embed)
                                await User.send(embeds=embed_list)
...
class Sub_button(discord.ui.View,):
    ...
    async def New_Sub(self, interaction: discord.Interaction, Button: discord.ui.Button):
        ...
        if User_ID in Sub_Discord_ID_list:
            await interaction.response.send_message(f"Já esta: {Team_name}", ephemeral=True)
        else:
            Sub_Discord_ID_list.append(User_ID)
            JSOn_data[f"{Team_name}"]["Sub_Discord_ID_list"] = Sub_Discord_ID_list
            fill_json_file(player_observation_data_dir, JSOn_data)

            text = f"Informarei você aqui por mensagem direta quando o status online de {Team_name} mudar."
            embed = discord.Embed(title=f"🔔 Agora você está linkado a equipe: {Team_name} 🔔", description=text, color=0xff8040)
            await User.send(embed=embed)

            text = f"Informarei você aqui por mensagem direta quando o status online de {Team_name} mudart."
            embed = discord.Embed(title=f"🔔 Agora você está linkado a equipe: {Team_name} 🔔", description=text, color=0xff8040)
            await interaction.response.send_message(embed=embed, ephemeral=True)
    ...
    async def deltSub(self, interaction: discord.Interaction, Button: discord.ui.Button):
        ...
        if User_ID in Sub_Discord_ID_list:
            ...
            text = f"Não Informarei  quando o status online den {Team_name} mudar."
            embed = discord.Embed(title=f"🔕 Agora você está deslinkado a equipe: {Team_name} 🔕", description=text, color=0xff0000)
            await User.send(embed=embed)

            text = f"Não Informarei  quando o status online de {Team_name} mudart."
            embed = discord.Embed(title=f"🔕 Agora você está deslinkado a equipe: {Team_name} 🔕", description=text, color=0xff0000)
            await interaction.response.send_message(embed=embed, ephemeral=True)
        else:
            await interaction.response.send_message(f"Voce já está linkado a {Team_name}.", ephemeral=True)

    @discord.ui.button(label="DeletarAll Links", style=discord.ButtonStyle.red, custom_id="UnSubAll")
    async def deltAllSubs(self, interaction: discord.Interaction, Button: discord.ui.Button):
        ...
        text = f" Voce não esta mais linkado a um time."
...
class all_players(commands.Cog):
    ...
    @app_commands.command(name="all_players", description="Mostra todos os players do server")
    async def all_players_list_send(self, interaction: discord.Interaction):
        ...
        for player in players[1:]:
            if len(current_chunk) + len(player) < 500:
                current_chunk += '\n' + player
            else:
        ...
        for i, chunk in enumerate(chunks):
            chunk_embed = discord.Embed(title=f"{data_len} Jogadores Online (Teil {i+1})", description=chunk)
...
class auto_smg_delt_player_observation(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if message.channel.id != player_observation_channel_id:
            ...
            print(f"Msg deletada em 10s: {message.author.name}:")
            print(f"msg:\n{message.content}\n")
            ...
            if message.content not in observation_commands:
                ...
                description = f"<@{message.author.id}>\nO canal de observação **não é um canal de bate-papo.**\nPor favor, utilize o canal designado para bate-papo, que é <#{rust_team_text_channel_id}>\n\n{delt_msg_str}\n\n**Neste canal, você pode usar os seguintes comandos:**"
                embed_attention = discord.Embed(title="Observação: canal de observação", description=description, colour=0x004000)
                ...
                content =f"Mensagem de <@{message.author.id}>\n> {message.content}"
