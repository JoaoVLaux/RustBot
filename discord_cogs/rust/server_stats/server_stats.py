...
class loops(commands.Cog, commands.Bot):
    ...
    async def myLoop(self, bot):
        ...
        if status_code == 200:
            ...
            current_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
            print (f"SKIP {loop_num}# - {current_time} - A contagem de jogadores não mudou!")
            if new_player_count:
                ...
                > **👥   Players: ({players}/{maxPlayers}) {player_channge_indi}** 
                > 🔄   Ultima Atualização: {Discord_time_stemp} 
                ...
                > 🛠️   Server: `{rust_type}`
                > 👥   Team: `{teamUILimit}` 
                > 📅   Ultimo Wipe: {last_wipe_discord_time} 
                > ⏰   Hora: `{timeZone}`
                """
                ...
                > [Rust Mapa Link]({map_url})
                """
                Server_card.add_field(name=f" ",
                            value=f"{text}",
                            inline=True)
                
                Server_card.add_field(name=f"Connect do server:",
                            value=f"```client.connect {server_ip_full}```",
                            inline=False)

                Server_card.set_image(url=server_headerimage)
                server_embeds.append(Server_card)
                Server_Description = discord.Embed(title="Descrição Server", colour=embed_side_colur, description=f"{rust_description}")
...
class change_server_id(commands.Cog):
    ...
    @app_commands.command(name="change_server", description="Troque Rust Server ID")
    @app_commands.describe(
        server_id="Rust Server ID do Battlemetrics")
    async def Server_change(
        ...
        print(f"Discord Comando: \"/change_server\" Novo Server ID [{server_id}]")
        print(f"Enviar Confirmação / Cancelar query.")

        print("Enviar Discordembed: Teste")
        await view.wait()
    ...
    async def on_message(self, message):
        ...
        if message.channel.id != server_stats_channel_id:
            ...
            if message.content not in ["/change_server"]:
                ...
                description = f"<@{message.author.id}>\nO canal Server Stats **não é um canal de bate-papo.**\nUtilize o canal designado para bate-papo, que é <#{rust_team_text_channel_id}>\n\n{delt_msg_str}\n\n**Neste canal, você pode use os seguintes comandos: **"
                embed_attention = discord.Embed(title="Observação: uso de Stats Server", description=description, colour=0x004000)
                ...
                embed.add_field(name="```/change_server```",
                                value="Altere o servidor do bot inserindo o ID Battlemetrics do servidor",
                                inline=False)
