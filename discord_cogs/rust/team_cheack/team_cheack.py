...
class bot_team_cheack(commands.Cog):
    ...
    description = "Detecta times Rust"

    @app_commands.command(name="team_check", description=description)
    @app_commands.describe(player_steam_url="O SteamID de um jogador ou  URL Steam",
    )
    async def choise_team(self, interaction: discord.Interaction, player_steam_url: str,):
        ...
        await interaction.response.send_message("Um momento por favor. Estou procurando os possíveis times deste jogador.", ephemeral=True)
        ...
        if data_steam_name == None:
            embed = discord.Embed(title="Team Check", url=player_steam_url,
                                  description="Nenhum dado encontrado neste URL do Steam.", color=0xff0006)
            ...
            if team_player_data_len == 1:
                embed = discord.Embed(
                    title="Team Check", description="Nenhuma informação sobre um time, talvez é solo", color=0x0080ff)
            else:
                embed = discord.Embed(
                    title="Team Check", description=f"A equipe é composta por {team_player_data_len} players.", color=0x0080ff)
            ...
            if offline == True:
                embed = discord.Embed(title="Team Check", url=player_steam_url,
                                      description="Jogador não encontrado. talvez não ta online no servidor agora", color=0xff0006)
                await interaction.response.send_message(embed=embed, ephemeral=True)
            else:
                view = Confirm_say()
                ...
                if view.value is None:
                    ...
                    embed = discord.Embed(title="Selecione uma equipe ou crie uma nova", description=f"{thinking} Pensando...", color=discord.Color.blurple())
                    ...
                    x = -1
                    options = [discord.SelectOption(label=f"🆕Criar um novo time.🆕", description=f"Adicionar um novo time."),]
                    while True:
                        x = x + 1
                        if x == Team_list_len:
                            break
                    ...
                    async def my_callback(interaction):
                        ...
                        if f"{select.values[0]}" == f"🆕Criar um novo time.🆕":
                            await interaction.response.send_modal(modal_New_team())

                        else:

                            embed = discord.Embed(title=f"Team Cheack", description="Observação do jogador adicionada.", color=0xff8040)
                            await interaction.response.send_message(embed=embed, ephemeral=True,)
                    ...
                    self.confirm_Button = False
                    print(
                        f'Cancelado... self.confirm_Button = {self.confirm_Button}')
                    # return self.confirm_Button, self.say_channel_id, self.say_title, self.say_text


class modal_New_team(ui.Modal, title="Novo time", ):

    New_Team_name = ui.TextInput(label="Nome do time", style=discord.TextStyle.short,
                                 placeholder="Team name", required=True, max_length=None)
    New_team_note = ui.TextInput(label="Adicionar nota:", style=discord.TextStyle.short,
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        print("Enviar Discordembed: Test")
        await view.wait()
        if view.value is None:
            self.confirm_Button = False
